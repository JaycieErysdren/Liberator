...
class SlavedriverLevQuake(KaitaiStruct):
    ...
    def _read(self):
        ...
        self.sky_data = SlavedriverLevQuake.SkyDataT(_io__raw_sky_data, self, self._root)
        self.header = SlavedriverLevQuake.HeaderT(self._io, self, self._root)
        self.sectors = []
        for i in range(self.header.num_sectors):
            self.sectors.append(SlavedriverLevQuake.SectorT(self._io, self, self._root))

        self.planes = []
        for i in range(self.header.num_planes):
            self.planes.append(SlavedriverLevQuake.PlaneT(self._io, self, self._root))

        self.tiles = []
        for i in range(self.header.num_tile_entries):
            self.tiles.append(SlavedriverLevQuake.TileT(self._io, self, self._root))

        self.vertices = []
        for i in range(self.header.num_vertices):
            self.vertices.append(SlavedriverLevQuake.VertexT(self._io, self, self._root))

        self.quads = []
        for i in range(self.header.num_quads):
            self.quads.append(SlavedriverLevQuake.QuadT(self._io, self, self._root))

        self.entities = []
        for i in range(self.header.num_entities):
            self.entities.append(SlavedriverLevQuake.EntityT(i, self._io, self, self._root))

        self.entity_polylinks = []
        for i in range(self.header.num_entity_polylinks):
            self.entity_polylinks.append(SlavedriverLevQuake.EntityPolylinkT(self._io, self, self._root))
        ...
        self.entity_polylink_data1 = SlavedriverLevQuake.EntityPolylinkData1T(_io__raw_entity_polylink_data1, self, self._root)
        ...
        self.entity_polylink_data2 = SlavedriverLevQuake.EntityPolylinkData2T(_io__raw_entity_polylink_data2, self, self._root)
        ...
        self.entity_data = SlavedriverLevQuake.EntityDataT(_io__raw_entity_data, self, self._root)
        ...
        self.tile_texture_data = SlavedriverLevQuake.TileTextureDataT(_io__raw_tile_texture_data, self, self._root)
        ...
        self.tile_color_data = SlavedriverLevQuake.TileColorDataT(_io__raw_tile_color_data, self, self._root)
        self.unknown = []
        for i in range(self.header.num_unknown):
            self.unknown.append(self._io.read_bytes(128))

        self.resources = SlavedriverLevQuake.ResourcesT(self._io, self, self._root)
        self.unknown0 = SlavedriverLevQuake.LenAndUnknownT(self._io, self, self._root)
        self.level_name = (self._io.read_bytes(32)).decode(u"ASCII")
        self.unknown1 = []
        for i in range(9):
            self.unknown1.append(self._io.read_u4be())

        self.unknown2 = SlavedriverLevQuake.LenAndUnknownT(self._io, self, self._root)
        self.unknown3 = SlavedriverLevQuake.LenAndUnknownT(self._io, self, self._root)
    ...
    class EntityT(KaitaiStruct):
        ...
        def get_entity_data(self):
            ...
            if self.ofs_entity_data < self._root.header.len_entity_data:
                ...
                _on = self.ent_type
                if _on == 146:
                    self._m_get_entity_data = SlavedriverLevQuake.EntityPolymoverT(io, self, self._root)
                else:
                    self._m_get_entity_data = SlavedriverLevQuake.EntityGenericT(io, self, self._root)
                io.seek(_pos)
    ...
    class ResourcesT(KaitaiStruct):
        ...
        def _read(self):
            self.prefix = SlavedriverLevQuake.ResourcesPrefixT(self._io, self, self._root)
            self.num_sounds = self._io.read_u4be()
            self.sounds = []
            for i in range(self.num_sounds):
                self.sounds.append(SlavedriverLevQuake.SoundT(self._io, self, self._root))

            self.len_palette = self._io.read_u4be()
            self.palette = []
            for i in range(self.len_palette // 2):
                self.palette.append(SlavedriverLevQuake.PaletteEntryT(self._io, self, self._root))

            self.num_resources = self._io.read_u4be()
            self.resources = []
            for i in range(self.num_resources):
                self.resources.append(SlavedriverLevQuake.ResourceT(self._io, self, self._root))
    ...
    class ResourceT(KaitaiStruct):
        ...
        def _read(self):
            ...
            if _on == 130:
                self.data = SlavedriverLevQuake.TextureT(self._io, self, self._root)
            elif _on == 52:
                self.data = SlavedriverLevQuake.Resource0x34T(self._io, self, self._root)
            elif _on == 106:
                self.data = SlavedriverLevQuake.Resource0x6aT(self._io, self, self._root)
            elif _on == 108:
                self.data = SlavedriverLevQuake.Resource0x6cT(self._io, self, self._root)
    ...
    class SkyDataT(KaitaiStruct):
        ...
        def _read(self):
            ...
            self.bitmaps = []
            for i in range(64):
                self.bitmaps.append(SlavedriverLevQuake.SkyBitmapT(self._io, self, self._root))
